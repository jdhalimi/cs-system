...
```text
citya-docs export → exports Citya → citya-docs capture → validation → Drive
```

1. `citya-docs export --new` télécharge les documents inconnus dans les exports, avec
   leur date réelle (`dateCreated` Citya, pas la date de téléchargement).
2. `citya-docs capture` affiche le plan d'envoi et les empreintes des fichiers.
3. Vérifier ce plan.
4. Relancer `citya-docs capture --apply` pour envoyer les fichiers vers Drive.
...
`google-drive index` a trouvé sur Drive — sauf si sa date locale est plus récente que
la date de dépôt sur Drive à ce même chemin, auquel cas il est repris dans le
plan (probable mise à jour côté Citya sous le même nom).

Lancer `google-drive index` seul pour rafraîchir `data/exports/google/drive/manifest.json`
sans envoyer de document, par exemple après une synchronisation manuelle.
...
`google-contacts export` exporte les Google Contacts vers
...
`notion-contacts export` exporte les bases Notion Proprietaires et Locataires ;
`notion-lots export` exporte la base Lots. Les trois sont indépendantes des
Google Contacts et ne nécessitent qu'un jeton Notion.
